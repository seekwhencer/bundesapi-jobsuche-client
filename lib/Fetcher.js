...
export default class Fetcher {
    ...
    async props(options) {
        ...
        props.outputPath = `${props.dataPath}/jobs`;
        props.searchPath = `${props.dataPath}/search`;
    ...
    async search(options) {
        ...
        const jobsCount = await this.fetchAll(props).catch(err => console.error('❌ Fehler:', err.message));

        console.log(`\n✅ Fertig. Insgesamt ${jobsCount} Jobs verarbeitet.`);
        console.log(`📂 Ordner: ${props.outputPath}\n`);
    ...
    async fetchAll(props) {
        console.log(`🔍 Suche Jobs in "${props.location}" (Umkreis ${props.radius} km)${props.search ? ` für "${props.search}"` : ''}${parseInt(props.days) > 0 ? `, aktualisiert in den letzten ${props.days} Tagen` : ''}...\n`);

        let totalCount = 0;
        let page = 1;

        while (true) {
            console.log(`➡️  Lade Seite ${page}...`);
            let jobs = [];
            try {
                jobs = await this.fetchPage(page, props);
            } catch (err) {
                console.error(`⚠️ Fehler bei Seite ${page}: ${err.message}`);
                break;
            }
            ...
            for (let i = 0; i < jobs.length; i += limit) {
                ...
                await Promise.all(chunk.map(async (j) => {
                    ...
                    if (await fs.pathExists(jobFile)) {
                        console.log(`⚙️  Überspringe vorhandene Job-Details ${j.refnr}`);
                    ...
                    console.log(`💾 Gespeichert: ${j.refnr}`);
                    totalCount++;
                }));
            }
            page++;
