...
export default class Server extends Events {
    ...
    registerRoutes() {
        ...
        this.app.ws('/logs', (ws, req, next) => {
            const hashBase = Date.now().toString();
            const id = crypto.createHash('md5').update(hashBase).digest('hex').slice(0, 12);
            const metadata = {id};
            this.wsClients.set(ws, metadata);

            console.log('>>> WEBSOCKET CONNECT:', metadata.id);
    ...
    sendWS(message) {
        ...
        [...this.wsClients.keys()].forEach((client) => client.send(json));
    }
    ...
    get scheduler() {
        return this.jobbinger.scheduler;
    }

    set scheduler(server) {
        //
    }
}